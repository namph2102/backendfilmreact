...
class CommemtController {
  ...
  async handleAddCommemt(req, res) {
    try {
      ...
      if (subcomment) {
        //nếu là commem bậc 2 trơ lên thì
        const dataParent = await CommemtModel.findByIdAndUpdate(subcomment, {
        ...
        const allSubcommemt = await CommemtModel.find({
